...
![群系](https://github.com/codeduckerxyx/Data_structure_and_algorithm/blob/main/%E6%95%B0%E6%8D%AE%E7%BB%93%E6%9E%84/%E7%BE%A4%E7%B3%BB.png)
...
![二维前缀和](C:\Users\xyx\Documents\makedown\数据结构\二维前缀和.png)
...
inline int query(int L,int R){
    ...
    return max( f[L][ len2 ],f[R-(1<<len2)+1][ len2 ] );
}
```
...
<img src="C:\Users\xyx\Documents\makedown\数据结构\树状数组.png" alt="树状数组" style="zoom: 67%;" />
...
<img src="C:\Users\xyx\Documents\makedown\数据结构\前缀和与差分.png" alt="前缀和与差分" style="zoom: 80%;" />

> 由上面的公式,我们可以构造一个差分数组建立一个树状数组,然后我们再构造一个i*bi的树状数组

```cpp
ll c1[500005];//b[i]差分构造的树状数组
ll c2[500005];//b[i]*i构造的树状数组
...
<img src="C:\Users\xyx\Documents\makedown\数据结构\线段树.png" alt="线段树" style="zoom: 80%;" />
...
int main(){
    ...
    rep(i,1,n) scanf("%lld",&node[i].val);
    int u,v ;
    rep(i,1,n-1){
        scanf("%d%d",&u,&v);
        addEdge(u,v,0);
