...
bool cmp1(cpp &tema,cpp &temb){
    return tema.val<temb.val;
}
bool cmp2(cpp &tema,cpp &temb){
    return tema.id<temb.id;
}
void lsh(cpp *arr,int len){//第一个元素的指针和数组长度
...
```cpp
int bsearch(int L, int R)//右边满足条件
{
    int M;
    while (L < R)
    {
        M = (L + R) >> 1;
        if (check(M)) R = M;    // check()判断mid是否满足性质
        else L = M + 1;
    }
    return L;
}
int bsearch(int L, int R)//左边满足条件
{
    int M;
    while (L < R)
    {
...
s.erase(item)//删除 s 中对应元素 O(logn),多重集会全删
